...
def async_cmd(args, verbose=False, executable='/bin/sh', strict=False):
    lines = []
    if os.name != 'nt':
    ...
    if strict and return_val != 0:
        error_details = "".join(lines)
        raise RuntimeError(f"Command execution violently failed with OS code {return_val}. Traces: {error_details}")
