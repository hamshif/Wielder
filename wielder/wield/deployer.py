...
from wielder.util.kube_auth import new_kubernetes_api_client
...
def delete_multiple(res_tuples, module_root, context):
    ...
    for res_tup in res_tuples:
        ...
        if _type == KubeResType.PV:
            ...
            os.system(
                f'kubectl --context {context} delete -f {module_root}/{res_path} --ignore-not-found=true --wait=false;'
            )
            ...
            if _type == KubeResType.DEPLOY or _type == KubeResType.STATEFUL_SET:
                ...
                for pod in pods:
                    os.system(
                        f"kubectl --context {context} delete -n {namespace} pod/{pod.metadata.name} "
                        f"--ignore-not-found=true --force --grace-period=0;"
                    )
...
def get_pods(name, context, exact=False, namespace='default'):
    ...
    api_client = new_kubernetes_api_client(context=context)
