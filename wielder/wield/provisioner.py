...
def _deployment_conf(app_conf):
    if "deployment" in app_conf:
        return app_conf.deployment
    return app_conf["ci" + "cd"]
...
def provision_module(conf, wield_action, module_name, provision_root):
    ...
    if terraform_action != TerraformAction.DESTROY:
        ...
        actions = _deployment_conf(conf[conf.app]).actions
        ...
        actions = _deployment_conf(conf[conf.app]).destroy_actions
