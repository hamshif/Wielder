#!/usr/bin/env python
import logging
import os
import pyhocon
import wielder.util.util as wu

from pyhocon import ConfigFactory

from wielder.util.bucketeer import AWSBucketeer, get_bucketeer, get_ecosystem_bucketeer
from wielder.wield.enumerator import local_deployments, RuntimeEnv
from wielder.wield.project import configure_external_kafka_urls


def configure_remote_unique_context_ecosystem(conf, bucket_name=None):
    """
    Ecosystem-native bucket upload path. Uses the mapped ecosystem bucketeer directly.
    """
    if bucket_name is None:
        bucket_name = conf.namespace_bucket

    configure_external_kafka_urls(conf)
    unique_name = conf.unique_name
    serialized_conf = pyhocon.HOCONConverter.to_hocon(conf)
    artifactory_bucket = conf.artifactory_bucket
    unique_config_path = conf.unique_config_path
    logging.info(f'Configuring experiment {unique_name} at:\n{unique_config_path}')
    bucketeer = get_ecosystem_bucketeer(conf)
    bucket_names = bucketeer.get_bucket_names()
    aws_region = conf.aws_region if conf.runtime_env == 'aws' else None

    if bucket_name not in bucket_names:
        bucketeer.create_bucket(bucket_name=bucket_name, region=aws_region)

    if artifactory_bucket not in bucket_names:
        bucketeer.create_bucket(bucket_name=artifactory_bucket, region=aws_region)

    bucketeer.upload_string(
        serialized_conf,
        bucket_name,
        f'{unique_config_path}/{unique_name}.conf',
    )


def get_remote_unique_context_ecosystem(conf):
    """
    Ecosystem-native bucket read path. Reads the remote config directly from the mapped ecosystem bucketeer.
    """
    namespace_bucket = conf.namespace_bucket
    unique_name = conf.unique_name
    unique_config_path = conf.unique_config_path

    file_name = f'{unique_name}.conf'
    bucketeer = get_ecosystem_bucketeer(conf)
    with bucketeer.read_object_stream(namespace_bucket, unique_config_path, file_name) as stream:
        conf = ConfigFactory.parse_string(stream.read())

    return conf


def configure_remote_unique_context(conf, bucket_name=None):
    """
    Legacy remote configuration publisher preserved for migrating clients.
    """

    if bucket_name is None:
        bucket_name = conf.namespace_bucket

    configure_external_kafka_urls(conf)

    unique_name = conf.unique_name

    app_plan_dir = f'{conf.project_conf_root}/plan'
    wu.makedirs(app_plan_dir, exist_ok=True)
    unique_context_conf = f'{app_plan_dir}/{unique_name}.conf'

    with open(unique_context_conf, "w") as outfile:
        outfile.write(pyhocon.HOCONConverter.to_hocon(conf))

    artifactory_bucket = conf.artifactory_bucket
    unique_config_path = conf.unique_config_path
    logging.info(f'Configuring experiment {unique_name} at:\n{unique_config_path}')

    if conf.runtime_env in local_deployments:
        bucket_path = conf.local_buckets_root
        conf_path = f'{bucket_path}/{bucket_name}/{unique_config_path}'
        wu.makedirs(conf_path, exist_ok=True)
        wu.copyfile(unique_context_conf, f'{conf_path}/{unique_name}.conf')

    elif conf.runtime_env == 'aws':
        aws_region = conf.aws_region
        bucketeer = AWSBucketeer(conf)
        bucket_names = bucketeer.get_bucket_names()

        if bucket_name not in bucket_names:
            bucketeer.create_bucket(bucket_name=bucket_name, region=aws_region)

        if artifactory_bucket not in bucket_names:
            bucketeer.create_bucket(bucket_name=artifactory_bucket, region=aws_region)

        bucketeer.upload_file(
            unique_context_conf,
            bucket_name,
            f'{unique_config_path}/{unique_name}.conf',
            is_text=True
        )


def get_remote_unique_context(conf):
    """
    Legacy remote configuration reader preserved for migrating clients.
    """
    namespace_bucket = conf.namespace_bucket
    unique_name = conf.unique_name
    unique_config_path = conf.unique_config_path

    bucket_path = conf.local_buckets_root
    conf_path = f'{bucket_path}/{namespace_bucket}/{unique_config_path}'
    file_name = f'{unique_name}.conf'

    if conf.runtime_env not in local_deployments:
        runtime_env = RuntimeEnv[conf.bootstrap_env.upper()]
        bucket_env = RuntimeEnv[conf.runtime_env.upper()]
        bucketeer = get_bucketeer(conf, runtime_env, bucket_env)
        bucketeer.download_object(namespace_bucket, key=unique_config_path, name=file_name, dest=conf_path)

    conf = wu.parse_file_as_hocon(f'{conf_path}/{file_name}')
    return conf


def download_stuff(conf):
    b = AWSBucketeer(conf)

    stuff = conf.stuff

    src_path = stuff.bucket_path
    bucket_name = stuff.bucket_name
    local_dest = stuff.local_dest

    os.system(f'atom {local_dest}')

    b.download_objects_by_key(
        bucket_name=bucket_name,
        root_key=src_path,
        dest=local_dest
    )


def download_stuff_one_object(conf):
    b = AWSBucketeer(conf)

    stuff = conf.stuff_one_object

    src_path = stuff.bucket_path
    bucket_name = stuff.bucket_name
    local_dest = stuff.local_dest

    # os.system(f'atom {local_dest}')

    b.download_objects(
        bucket_name=bucket_name,
        keys=[src_path],
        dest=local_dest
    )


def sync_stuff(conf):
    b = AWSBucketeer(conf)

    sync_stuff = conf.sync_stuff

    source = sync_stuff.source
    dest = sync_stuff.dest

    src_path = sync_stuff.src_path
    dest_path = sync_stuff.dest_path

    for stuff in sync_stuff.stuffs:

        src_key = f'{src_path}/{stuff}'
        dest_key = f'{dest_path}/{stuff}'

        b.bucket_sync(
            source=source,
            dest=dest,
            src_prefix=src_key,
            dest_prefix=dest_key
        )


def upload_stuff(conf):
    b = AWSBucketeer(conf)

    stuff = conf.upload

    bucket_path = stuff.bucket_path
    bucket_name = stuff.bucket_name
    src = stuff.src

    b.upload_directory(src, bucket_name, bucket_path)
